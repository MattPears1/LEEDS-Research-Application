...
```{r Boxplotsplits1, echo=FALSE, message=FALSE, warning=FALSE}
...
Boxplotsplits1 %>%
  ...
  filter(UseChatbotsFrequently %in% c("Strongly Agree","Agree","Neutral","Disagree","Strongly Disagree")) %>%
           
           drop_na(UseChatbotsFrequently) %>%
  drop_na(sex)%>%
#------then pipe into ggplot
  #the first aesthetic label is always the x axis
  ggplot(aes(sex))+
  geom_bar(aes(fill = sex), alpha = 0.8)+
  facet_wrap(~UseChatbotsFrequently)+
  theme_get()+
...
Boxplotsplits3 %>%
  ...
  filter(CEPEH_chatbots_easy_use_EE3POST %in% c("Strongly Agree","Agree","Neutral","Disagree","Strongly Disagree")) %>%
           
           drop_na(CEPEH_chatbots_easy_use_EE3POST) %>%
  drop_na(CEPEH_chatbots_easy_use_EE3POST)%>%
#------then pipe into ggplot
  #the first aesthetic label is always the x axis
  ggplot(aes(ChatbotUsed))+
  geom_bar(aes(fill = ChatbotUsed), alpha = 0.8)+
  facet_wrap(~CEPEH_chatbots_easy_use_EE3POST)+
  theme_get()+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.title.x=element_blank(),
...
```{r SUS table 2 questions, echo=FALSE, message=FALSE, warning=FALSE}
...
t.test(before, after, mu=0, alt="two.sided", paired=T, conf.level=0.095)
