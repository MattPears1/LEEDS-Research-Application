...
curr<- na.omit(curr)
colnames(curr)[1] = "CurrentMaterials"

knitr::kable(curr<-curr%>% filter(CurrentMaterials %in% c("Books","Online videosinteractive material", "Online journals","books","One to one workshops or lectures", format('html'))) %>% group_by(CurrentMaterials) %>% count(CurrentMaterials),"pipe")
...
```{r TEST Boxplotsplits1, echo=TRUE, message=FALSE, warning=FALSE}
