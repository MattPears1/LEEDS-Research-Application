...
```{r this half worked but keep, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r group by word and filter for true, message=FALSE, warning=FALSE}
CEPEHQ_tidy %>% 
  group_by(doc_id) %>% 
  count(word, sort=TRUE) %>% 
  filter(doc_id == "1")
...
CEPEHQ_freq <- CEPEHQ_freq %>% 
  na.omit() %>% 
  group_by(doc_id) %>% 
  mutate(pmw = n*1000/sum(n)) %>% # creates a new column called pmw
  ungroup() %>% 
...
ggplot(aes(x=total_score, y=total_score, group=total_score), data=CEPEHQ_tidy_bing) +
    geom_violin(aes(color=total_score), show.legend=FALSE)
```
