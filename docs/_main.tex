...
\NormalTok{Boxplotsplits1 }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"C:/Users/MattP/Desktop/cepeh R/Full DATA CEPEH Moh {-} Copy.csv"}\NormalTok{, }\AttributeTok{header=}\ConstantTok{TRUE}\NormalTok{, }\AttributeTok{na.strings=}\FunctionTok{c}\NormalTok{(}\StringTok{""}\NormalTok{, }\StringTok{"NA"}\NormalTok{))}

\CommentTok{\#{-}{-}{-}{-}{-}{-}make ggplot this way, instead of making a dataset, this is better as you can just filter and skips steps}
\NormalTok{Boxplotsplits1 }\SpecialCharTok{\%\textgreater{}\%}
  
  \FunctionTok{filter}\NormalTok{(Location }\SpecialCharTok{\%in\%} \FunctionTok{c}\NormalTok{(}\StringTok{"Cyprus"}\NormalTok{,}\StringTok{"Greece"}\NormalTok{,}\StringTok{"Sweden"}\NormalTok{,}\StringTok{"United Kingdom"}\NormalTok{)) }\SpecialCharTok{\%\textgreater{}\%}
           
           \FunctionTok{drop\_na}\NormalTok{(Location) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{drop\_na}\NormalTok{(Profession)}\SpecialCharTok{\%\textgreater{}\%}
\CommentTok{\#{-}{-}{-}{-}{-}{-}then pipe into ggplot}
  \CommentTok{\#the first aesthetic label is always the x axis}
  \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(Profession))}\SpecialCharTok{+}
  \FunctionTok{geom\_bar}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{fill =}\NormalTok{ Profession), }\AttributeTok{alpha =} \FloatTok{0.8}\NormalTok{)}\SpecialCharTok{+}
  \FunctionTok{facet\_wrap}\NormalTok{(}\SpecialCharTok{\textasciitilde{}}\NormalTok{Location)}\SpecialCharTok{+}
  \FunctionTok{theme\_get}\NormalTok{()}\SpecialCharTok{+}
  \FunctionTok{theme}\NormalTok{(}\AttributeTok{panel.grid.major =} \FunctionTok{element\_blank}\NormalTok{(),}
        ...
        \AttributeTok{axis.ticks.x=}\FunctionTok{element\_blank}\NormalTok{())}\SpecialCharTok{+}
                 \FunctionTok{labs}\NormalTok{(}\AttributeTok{title=}\StringTok{"Location and Profession of Participants"}\NormalTok{,}
                      \AttributeTok{x=} \StringTok{"Like to use chatbots frequently (Pre)"}\NormalTok{,}
...
Therefore, we can state that the sample used did not regularly use chatbots for their course learning, with 18/42 having used a chatbot at least once for between 0-4 hours of ue in total.
...
\includegraphics{_main_files/figure-latex/TEST Boxplotsplits1-1.pdf}
