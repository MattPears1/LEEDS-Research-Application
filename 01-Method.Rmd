...
body { background-color: lavenderblush; }
    pre, pre:not([class]) { background-color: lavenderblush; }text-align: justify}
</style>
...
Therefore, there are 42 points of data in the condition before testing, and 126 data points after testing the chatbots- for a total of `r count(data)` row of data. There were 78 questions asked in total, therefore the full dataset had approximately 6000 cells recorded.
...
```{r location1, echo=FALSE, fig.cap="Location and Profession of Participants", message=FALSE, warning=FALSE}
...
Multiple metrics are more appropriate when measuring usability of chatbots ​[6]​ therefore a combination of questions from multiple measures can help internal consistency and interpret the results more approporiately. 
