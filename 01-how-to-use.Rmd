...
This dataset had `r count(Males)` males and `r count(Females)` females, for a total of `r count(data)`. It was a repeated measure design therefore`

There were `r count(Femalesgreece)` females from Greece, `r count(Femalescyprus)` from Cyprus, `r count(Femalessweden)` from Sweden.\

There were `r count(malesgreece)` males from Greece, `r count(malescyprus)` from Cyprus, `r count(malessweden)` from Sweden.
