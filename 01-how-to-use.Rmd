...
curr <- subset(data, select = c("Current_Materials"))
curr <- na.omit(curr)
colnames(disc)[1] = "Current_Materials"

knitr::kable(curr <-curr %>% filter(Current_Materials %in% c("Books","Online videos/interactive material", "Online journals/books","1 to 1 workshops or lectures",format('html'))) %>% group_by(Current_Materials) %>% count(Current_Materials),"pipe")
